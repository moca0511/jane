...
void int_cmt0(void)
{
	...
	*****************************************************************************************/
	...
	if(run_mode == STRAIGHT_MODE){
		...
		if(tar_speed < max_speed*-1){
			tar_speed = max_speed*-1;
		}
	...
	*****************************************************************************************/
	//フィードバック制御
	V_r = V_l = 0.0;
	if(run_mode == STRAIGHT_MODE || run_mode == TURN_MODE || run_mode == F_WALL_MODE){
	//直進時のフィードバック制御
		//左右モータのフィードバック
		//速度に対するP制御
	...
	switch(state)
	{
		...
		case 1:		//前左センサ読み込み
			...
			if(sen_fl.value > sen_fl.th_wall)		//壁の有無を判断
			{
				...
				sen_fl.error = sen_fl.value - sen_fl.ref;	//制御をかける場合は偏差を計算
				sen_fl.is_control = true;		//左センサを制御に使う
			}
			else
			{
				sen_fl.is_wall = false;			//左前壁なし
				sen_fl.is_control = false;		//左センサを制御に使う
		...
		case 2:		//前右センサ読み込み
			...
			if(sen_fr.value > sen_fr.th_wall)		//壁の有無を判断
			{
				...
				sen_fr.error = sen_fr.value - sen_fr.ref;	//制御をかける場合は偏差を計算
				sen_fr.is_control = true;		//左センサを制御に使う
			}
			else
			{
				sen_fr.is_wall = false;			//右前壁なし
				sen_fr.is_control = false;		//左センサを制御に使う
