			...
			case 4:
				...
				*****************************************/
				...
				if(sen_fr.value + sen_fl.value + sen_r.value + sen_l.value > SEN_DECISION * 4){
					BEEP();
					LED(9);
					//左右に壁がある場所で中央においてスタート
					if(sen_fr.value + sen_fl.value + sen_r.value + sen_l.value > SEN_DECISION * 4){
						BEEP();
						wait_ms(500);
						sen_r.ref=sen_r.value;
						sen_l.ref=sen_l.value;
					}
					BEEP();
					LED(8);
					//左右に壁がある場所で右端においてスタート
					if(sen_fr.value + sen_fl.value + sen_r.value + sen_l.value > SEN_DECISION * 4){
						BEEP();
						wait_ms(500);
						sen_l.th_control = sen_l.th_wall=sen_l.value;
					}
					BEEP();
					LED(1);
					//左右に壁がある場所で左端においてスタート
					if(sen_fr.value + sen_fl.value + sen_r.value + sen_l.value > SEN_DECISION * 4){
						BEEP();
						wait_ms(500);
						sen_r.th_control = sen_r.th_wall=sen_r.value;
					}
					BEEP();
					LED(6);
					//前方に壁がある場所で区画の境目においてスタート
					if(sen_fr.value + sen_fl.value + sen_r.value + sen_l.value > SEN_DECISION * 4){
						BEEP();
						wait_ms(500);
						sen_fr.th_wall=sen_fr.value;
						sen_fl.th_wall=sen_fl.value;
					}
					BEEP();
				}
